...
public class WordBank {
    // Map to store word lists categorized by difficulty level (easy, medium, hard)
    private final Map<String, List<String>> wordMap = new HashMap<>();

    // Map to store hints for each word (each word may have multiple hints)
    private final Map<String, List<String>> hintMap = new HashMap<>();

    // Random object to randomly pick words
    private final Random random = new Random();

    // Constructor - initializes the word bank and hints when an object is created
    public WordBank() {
        initializeWords(); // Fill the wordMap with categorized words
        initializeHints(); // Fill the hintMap with corresponding hints
    }

    // Method to initialize the words in different difficulty categories
    ...
    // Method to initialize hints for all words
    private void initializeHints() {
        // Each call to addHint adds a hint string for a specific word
        // Easy words
    ...
    // Helper method to add a hint for a word
    private void addHint(String word, String hint) {
        // If the word is not already in the map, create a new list
        hintMap.computeIfAbsent(word, k -> new ArrayList<>()).add(hint);
    }

    // Returns a random word from the given difficulty category
    ...
    // Returns the list of hints for a given word
